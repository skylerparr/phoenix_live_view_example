defmodule Accounts.PlayerManager do
  ...
  def handle_call(
        {:add_player, session_pid, player},
        _,
        {map_by_id, map_by_session_pid, by_session_id}
      ) do
  ...
  def handle_call(
        {:get_player_by_session_pid, session_pid},
        _,
        {_map_by_id, map_by_session, _by_session_id} = state
      ) do
    {:reply, Map.get(map_by_session, session_pid), state}
  end

  def handle_call(
        {:get_player_by_session_id, session_id},
        _,
        {_map_by_id, _map_by_session, by_session_id} = state
      ) do
